...
LATEST_VERSION=$(curl --fail --show-error --location -s https://api.github.com/repos/kunalsin9h/meltcd/releases/latest | grep "tag_name" | cut -d '"' -f 4)

status "Downloading MeltCD v$LATEST_VERSION for $OS $ARCH"
curl --fail --show-error --location --progress-bar -o $TEMP_DIR/meltcd.tar.gz "https://github.com/kunalsin9h/meltcd/releases/download/$LATEST_VERSION/meltcd_${LATEST_VERSION}_${OS}_$ARCH.tar.gz"
